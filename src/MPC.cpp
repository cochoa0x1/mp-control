...
//mark the starting index for each state param in the variables list
const int x_start =0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //base on code from class notes
    fg[0]=0.0; //the starting cost

    //cost based on target values
    for (int t = 0; t < N; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //set everything that isnt an actuator to a very large value. anything goes
  for(int i =0; i < delta_start; i++){
    vars_lowerbound[i] = -1e15;
    vars_upperbound[i] = 1e15;
  }
